...
There are some conventions in the community that we need to be aware of
first of all, the first dimension of a tensor is always is batch_size, and all the algorithm just create their
 variables to be oblivious to the size of batch for example we create X to be the size of [None, 5] here None represent
the freedom of choosing different batch sizes
'''
...
def multilayer_rnn():
    ...
    cells = [tf.contrib.rnn.BasicRNNCell(n_neurons) for _ in range(n_layers)]
    multilayer_cell = tf.contrib.rnn.MultiRNNCell(cells)
    outputs, states = tf.nn.dynamic_rnn(multilayer_cell, X, dtype=tf.float32)
